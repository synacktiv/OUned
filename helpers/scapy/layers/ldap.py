...
class _SaslCredentialsField(ASN1F_STRING_PacketField):
    def m2i(self, pkt, s):
        ...
        if pkt.mechanism.val in _SASL_MECHANISMS:
            ...
            # The SASL "credentials" field might not always be a GSSAPI_BLOB. It might also
            # be a raw GSSAPI token. This is the case at the last step of SASL negotiation
            # using the GSSAPI mechanism, when the client sends a wrapped LDAP_SASL_GSSAPI_SsfCap
            except BER_Decoding_Error:
                return (
                    LSAP_SASL_GSSAPI_SsfCap_Wrapped(val[0].val, _underlayer=pkt),
                    val[1]
                )
        return val
...
class LDAP_FilterExtensibleMatch(ASN1_Packet):
    ...
    ASN1_root = ASN1F_SEQUENCE(
        ...
        ASN1F_optional(
            LDAPString("type", "", implicit_tag=0x82),
        ),
        AttributeValue("matchValue", "", implicit_tag=0x83),
        ASN1F_optional(
    ...
    def from_rfc2254_string(filter: str):
        ...
        def _getfld(x):
            if not x:
                ...
                # extensible - RFC 2251
                pattern = r"^(?P<type>[^:]+)?(?P<dn>:dn)?(?::(?P<matchingRule>[0-9.]+))?(?::=)(?P<value>.+)$"
                match = re.match(pattern, ''.join(x))
                if match:
                    parsed_extensible = match.groupdict()
                    # One of type or matchingRule must be present
                    if not parsed_extensible['type'] and not parsed_extensible['matchingRule']:
                        raise ValueError("Invalid extensible filter")
                    extensible_filter = LDAP_FilterExtensibleMatch(matchValue=ASN1_STRING(parsed_extensible['value']))
                    if parsed_extensible['type']:
                        extensible_filter.type = ASN1_STRING(parsed_extensible['type'])
                    if parsed_extensible['matchingRule']:
                        extensible_filter.matchingRule = ASN1_STRING(parsed_extensible['matchingRule'])
                    if parsed_extensible['dn']:
                        extensible_filter.dnAttributes = True
                    return extensible_filter
                else:
                    raise ValueError("Invalid extensible filter")
...
class ASN1F_INTEGER_FLAGS(ASN1F_INTEGER):
    def __init__(self, name, default, mapping, **kwargs):
        self.mapping = mapping
        super(ASN1F_INTEGER_FLAGS, self).__init__(name, default, **kwargs)
    ...
    def any2i(self, pkt, x):
        if isinstance(x, str):
            ...
            for flag in x.split("+"):
                flag = flag.strip()
                if flag in self.mapping:
                    val |= 1 << self.mapping.index(flag)
            x = val
        return super(ASN1F_INTEGER_FLAGS, self).any2i(pkt, x)

    def i2repr(self, pkt, x):
        if x is not None and getattr(x, "val", None) is not None:
            val = x.val
            res = []
            for i, flag in enumerate(self.mapping):
                if val & (1 << i):
                    res.append(flag)
            if not res:
                return repr(x)
            return "+".join(res) + " (" + str(val) + ")"
        return repr(x)
...
class LDAP(ASN1_Packet):
    ...
    ASN1_root = ASN1F_SEQUENCE(
        ...
        # LDAP v3 only
        ASN1F_optional(
            ASN1F_SEQUENCE_OF("Controls", [], LDAP_Control, implicit_tag=0xA0)
        ),
    )
    ...
    def tcp_reassemble(cls, data, *args, **kwargs):
        ...
        # We should be at message boundary with the "data" variable. So if we are receiving an LDAP message directly, this will skip,
        # and it will continue to skip as long as the message was not handled.
        # If we are starting the received data with something else than an LDAP message, we assume it is a SASL buffer. If we can't cast the received data
        # as an LDAP packet (which calls dispatch_hook to try and cast it as a SASL_Buffer), return None to continue receiving data, even though
        # if this not either an LDAP packet or a SASL buffer, I'm not sure what it could be then
        if data[0] != 0x30:
...
class LSAP_SASL_GSSAPI_SsfCap_Wrapped(Packet):
    fields_desc = [
        _GSSAPI_Field("credentials", LDAP_SASL_GSSAPI_SsfCap),
    ]
