...
class SMBStreamSocketMultiplexing(StreamSocket):
    """
    A special kind of SMBStreamSocket that supports multiple sessions
    [MS-SMB2] 3.3.5.2.7
    """

    def __init__(self, *args, **kwargs):
        self.queue = collections.deque()
        # We have a dict of sessions. There is also a default session to handle packets that are not yet part of a session
        self.sessions = {}
        self.sessions[0] = SMBSession()
        super(SMBStreamSocketMultiplexing, self).__init__(*args, **kwargs)
    
    def add_session(self, sessionId):
        self.sessions[sessionId] = SMBSession()
    ...
    def recv(self, x=None):
        ...
        if self.queue:
            ...
            pkt = super(SMBStreamSocketMultiplexing, self).recv(x)
        try:
        ...
        if pkt is not None and (
            ...
            # If the client returned a session ID but we do not have it yet, we proposed the ID in a SMB2_Session_Setup_Response
            # but we did not yet register the session. Treat this as default session 0
            if sessionId not in self.sessions.keys():
                sessionId = 0
            pkt = self.sessions[sessionId].in_pkt(pkt)
            pay = pkt[SMB2_Header].payload
            while SMB2_Header in pay:
                pay = pay[SMB2_Header]
                pay.underlayer.remove_payload()
        ...
        if (
            smbh
            and self.sessions[sessionId].Dialect
            and self.sessions[sessionId].SigningKey
            and self.sessions[sessionId].SigningRequired
            # [MS-SMB2] sect 3.2.5.1.3 Verifying the Signature
            # "The client MUST skip the processing in this section if any of:"
            # - [...] decryption in section 3.2.5.1.1.1 succeeds
            and not smbh._decrypted
            ...
            smbh.verify(
                self.sessions[sessionId].Dialect,
                self.sessions[sessionId].SigningKey,
                # SMB 3.1.1 parameters:
                SigningAlgorithmId=self.sessions[sessionId].SigningAlgorithmId,
                IsClient=False,
            )
        return pkt

    def send(self, x, Compounded=False, ForceSign=False, ForceEncrypt=False, sessionId=0, **kwargs):
        for pkt in self.sessions[sessionId].out_pkt(
            x, Compounded=Compounded, ForceSign=ForceSign, ForceEncrypt=ForceEncrypt
        ):
            return super(SMBStreamSocketMultiplexing, self).send(pkt, **kwargs)

    @staticmethod
    def select(sockets, remain=conf.recv_poll_rate):
        if any(getattr(x, "queue", None) for x in sockets):
            return [x for x in sockets if isinstance(x, SMBStreamSocketMultiplexing) and x.queue]
        return select_objects(sockets, remain=remain)
