...
class SMB_Server(Automaton):
    ...
    socketcls = SMBStreamSocketMultiplexing
    ...
    def __init__(self, shares=[], ssp=None, verb=True, readonly=True, *args, **kwargs):
        ...
        # Set options for default session
        self.ssp = ssp
        self.nextSessionId = 0x0001000000000015
        self.sessions[0].ssp = ssp
        self.sessions[0].SigningRequired = (
    ...
    def sessions(self):
        # session shorthand
        return self.sock.sessions
    
    @property
    def active_sessions(self):
        return sum(1 for session in self.sessions.values() if getattr(session, "isActive", None) == True)
    ...
    def on_negotiate(self, pkt):
        try:
            self.sessions[0].sspcontext, spnego_token = self.sessions[0].ssp.NegTokenInit2()
            ...
            if DialectRevision and DialectRevision & 0xFF != 0xFF:
                # Version isn't SMB X.???
                self.sessions[0].Dialect = DialectRevision
            cls = None
            if self.SMB2:
                # SMB2
            ...
            if self.SMB2:  # SMB2
                ...
                if SMB2_Header in pkt and pkt.SecurityMode.SIGNING_REQUIRED:
                    self.sessions[0].SigningRequired = True
                ...
                resp = self.smb_header.copy() / cls(
                    ...
                    SecurityMode=(
                        ...
                        if self.sessions[0].SigningRequired
                ...
                if DialectRevision >= 0x0311 and pkt.NegotiateContextsCount:
                    ...
                    for ngctx in pkt.NegotiateContexts:
                        if ngctx.ContextType == 0x0002:
                            ...
                            for ciph in ngctx.Ciphers:
                                ...
                                if tciph in self.sessions[0].SupportedCipherIds:
                                    # Common !
                                    self.sessions[0].CipherId = tciph
                                    self.sessions[0].SupportsEncryption = True
                                    break
                        elif ngctx.ContextType == 0x0008:
                            # SMB2_SIGNING_CAPABILITIES
                            for signalg in ngctx.SigningAlgorithms:
                                tsignalg = SMB2_SIGNING_ALGORITHMS.get(signalg, None)
                                if tsignalg in self.sessions[0].SupportedSigningAlgorithmIds:
                                    # Common !
                                    self.sessions[0].SigningAlgorithmId = tsignalg
                                    break
                    ...
                    resp.NegotiateContexts = [
                        ...
                        / SMB2_Preauth_Integrity_Capabilities(
                            # SHA-512 by default
                            HashAlgorithms=[self.sessions[0].PreauthIntegrityHashId],
                            Salt=self.sessions[0].Salt,
                        ),
                        # Encryption capabilities
                        SMB2_Negotiate_Context()
                        / SMB2_Encryption_Capabilities(
                            # AES-128-CCM by default
                            Ciphers=[self.sessions[0].CipherId],
                        ),
                        # Signing capabilities
                        SMB2_Negotiate_Context()
                        / SMB2_Signing_Capabilities(
                            # AES-128-CCM by default
                            SigningAlgorithms=[self.sessions[0].SigningAlgorithmId],
                        ),
                    ]
            else:
                # SMB1
                resp = self.smb_header.copy() / cls(
                    ...
                    SecurityMode=(
                        "SIGNING_ENABLED+SIGNING_REQUIRED"
                        if self.sessions[self.nextSessionId].SigningRequired
                        else "SIGNING_ENABLED"
                    ),
                    ServerTime=(time.time() + 11644473600) * 1e7,
                    ServerTimeZone=0x3C,
            ...
            if SMB2_Header in pkt:
                ...
                self.sessions[0].computeSMBConnectionPreauth(
    ...
    def update_smbheader(self, pkt):
        ...
        if (
            not self.sessions[0].SigningRequired
        ...
        if (self.sessions[0].Dialect or 0) >= 0x0311:
    ...
    @ATMT.action(receive_setup_andx_request_in_serving)
    def on_setup_andx_request(self, pkt, ssp_blob):
        try:
            sessionId = pkt.SessionId
            # If session ID is 0, we want a new session on the SMB connection.
            if sessionId == 0:
                self.sessions[self.nextSessionId] = copy.deepcopy(self.sessions[0])
                sessionId = self.nextSessionId
            self.sessions[sessionId].sspcontext, tok, status = self.sessions[sessionId].ssp.GSS_Accept_sec_context(
                self.sessions[sessionId].sspcontext, ssp_blob
            )
            self.update_smbheader(pkt)
            if SMB2_Session_Setup_Request in pkt:
                # SMB2
                # If we do not have a session ID yet, propose one
                if sessionId == self.nextSessionId:
                    self.smb_header.SessionId = self.nextSessionId
                else:
                    self.smb_header.SessionId = sessionId
            if status not in [GSS_S_CONTINUE_NEEDED, GSS_S_COMPLETE]:
                ...
                self.sessions[sessionId].SessionPreauthIntegrityHashValue = None
            else:
                # Negotiation
                if (
                    SMBSession_Setup_AndX_Request_Extended_Security in pkt
                    ...
                    if SMB2_Session_Setup_Request in pkt:
                        ...
                        if self.GUEST_LOGIN:
                            ...
                            self.sessions[sessionId].IsGuest = True
                            self.sessions[sessionId].SigningRequired = False
                        if self.ANONYMOUS_LOGIN:
                            resp.SessionFlags = "IS_NULL"
                        # [MS-SMB2] sect 3.3.5.5.3
                        if self.sessions[sessionId].Dialect >= 0x0300 and self.REQUIRE_ENCRYPTION:
            ...
            if status == GSS_S_CONTINUE_NEEDED:
                ...
                self.sessions[sessionId].computeSMBSessionPreauth(
            ...
            if status == GSS_S_COMPLETE:
                ...
                self.sessions[sessionId].computeSMBSessionKeys(IsClient=False)
                self.sessions[sessionId].isActive = True
                # We are registering the session and increasing the next session number
            ...
            if (
                self.sessions[sessionId].isActive
                and not self.sessions[sessionId].IsGuest
                and self.sessions[sessionId].Dialect >= 0x0300
                and self.REQUIRE_ENCRYPTION
            ):
                # [MS-SMB2] sect 3.3.5.5.3: from now on, turn encryption on !
                self.sessions[sessionId].EncryptData = True
                self.sessions[sessionId].SigningRequired = False
        except:
            import traceback
            traceback.print_exc()
    ...
    def should_serve(self):
        # Serve files. Initialize these values if they were not initialized by a previous session
        self.current_trees = getattr(self, "current_trees", {})
        self.current_handles = getattr(self, "current_handles", {})
        self.enumerate_index = getattr(self, "enumerate_index", {}) # used for query directory enumeration
        self.tree_id = getattr(self, "tree_id", 0)
        self.base_time_t = getattr(self, "base_time_t", self.current_smb_time())
        raise self.SERVING()
    ...
    def receive_logoff_request(self, pkt):
        if SMB2_Session_Logoff_Request in pkt:
            # Received a request to terminate the session. If we have only one active session, go back to NEGOTIATED. If we have more, we should continue SERVING
    ...
    def send_logoff_response(self, pkt):
        try:
            ...
            self.sessions[sessionId].isActive = False
        except:
            import traceback
            traceback.print_exc()
    ...
    def send_tree_connect_response(self, pkt, tree_name):
        ...
        if (
            self.sessions[sessionId].Dialect >= 0x0311
            and not self.sessions[sessionId].EncryptData
            and share.encryptdata
        ):
            if not self.sessions[sessionId].SupportsEncryption:
    ...
    def send_ioctl_response(self, pkt):
        ...
        if pkt.CtlCode == 0x11C017:
        ...
        elif pkt.CtlCode == 0x00140204 and self.sessions[sessionId].sspcontext.SessionKey:
            ...
            if (self.sessions[sessionId].Dialect or 0) < 0x0300:
                ...
                / SMB2_IOCTL_Response(
                    ...
                    Buffer=[
                        (
                            ...
                            SMB2_IOCTL_Validate_Negotiate_Info_Response(
                                GUID=self.GUID,
                                DialectRevision=self.sessions[sessionId].Dialect,
                                SecurityMode=(
                                    "SIGNING_ENABLED+SIGNING_REQUIRED"
                                    if self.sessions[sessionId].SigningRequired
                                    else "SIGNING_ENABLED"
                                ),
                                Capabilities=self.NegotiateCapabilities,
                            ),
