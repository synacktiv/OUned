...
class KerberosSSP(SSP):
    ...
    class CONTEXT(SSP.CONTEXT):
        ...
        def __init__(self, IsAcceptor, req_flags=None):
            ...
            self.U2U = False
            if IsAcceptor:
                self.SendSeqNum = RandNum(0, 0x7FFFFFFF)._fix()
                self.RecvSeqNum = RandNum(0, 0x7FFFFFFF)._fix()
            else:
                self.SendSeqNum = 0
                self.RecvSeqNum = 0
    ...
    def GSS_Accept_sec_context(
        ...
        if Context.state == self.STATE.INIT:
            ...
            return Context, KRB_GSSAPI_Token(
                        MechType="1.2.840.113554.1.2.2",  # Kerberos
                        innerToken=KRB_InnerToken(
                            TOK_ID=b"\x02\x00",
                            root=ap_rep
                        ),
