...
def server(*settings):
    try:
        msg = f" - client is querying its GPC (LDAP), forwarding to {settings[2]}:{settings[3]}" if settings[1] == 389 else f" - client is querying its GPT (SMB), forwarding to {settings[2]}:{settings[3]}"
